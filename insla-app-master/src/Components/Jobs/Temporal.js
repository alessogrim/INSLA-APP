...
export default class AddWorker extends Component {
  ...
  componentDidMount() {
    let action = this.props.navigation.state.params.estimacion.action;
    if (action === "edit") {
  ...
  saveUser = () => {
    ...
    if (
      ...
      if (age < 0 || age > 120) {
        ...
        Alert.alert("Guardar Empleado", "El empleado ya esta realizando otra labor", [
          {
            text: "Aceptar"
          }
        ]);
        ...
        Alert.alert("Guardar Empleado", "Un empleado no puede excederse más de 30 dias en esta labor", [
          {
            text: "Aceptar"
          }
        ]);
        ...
        if (action === "edit") {
          let worker = this.props.navigation.state.params.estimacion.doc;
          let reference = this.props.navigation.state.params.estimacion.estimation;
          ...
          fs.collection("employees")
            ...
            .then(function(querySnapshot) {
              querySnapshot.forEach(function(doc) {
                fs.collection("employees").doc(doc.id).update({status: true})
              });
            })
          ...
          fs.collection("soilWorkers")
            ...
            .then(() => {
              ...
              this.props.navigation.navigate("addSuelo",{identidad:this.state.identidad});
            });
        }
      }
    } else {
