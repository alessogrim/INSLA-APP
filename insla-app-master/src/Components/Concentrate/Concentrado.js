...
export default class Concentrado extends Component {
  ...
  render() {
    ...
    const TabConcentrates = this.state.dbconcentrates.map((doc, index) => {
      return (
        <TouchableOpacity avatar key={index} onPress={() =>this.props.navigation.navigate("addConcentrate",{farm:farm,insumo:doc,save:save})}>
            <ListItem 
              avatar 
              key={index}
              >
    ...
    return (
      <View style={styles.container}>
        <Header style={styles.headerStyle}>
          ...
          <Body>
            <Title style={styles.titleStyle}>CONCENTRADOS</Title>
          </Body>
          <Right />
        </Header>
        <Container>
